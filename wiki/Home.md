# Yabbyville Wiki

Welcome to the Yabbyville wiki. This covers each page and component so you can understand how the app fits together and how to customise it for your own community.

## Pages

- [Film Club Message](Pages-FilmClubMessage)
- [Home](Pages-Home)
- [List Detail](Pages-ListDetail)
- [Login](Pages-Login)
- [Lists](Pages-Lists)
- [Media Manager](Pages-MediaManager)
- [Message Board](Pages-MessageBoard)
- [News](Pages-News)
- [Profile](Pages-Profile)
- [Stickers](Pages-Stickers)
- [Travel](Pages-Travel)
- [Upload](Pages-Upload)
- [User Profile](Pages-UserProfile)
- [Wiki](Pages-Wiki)

## Components

### Basic (reusable UI building blocks)

- [AlbumSearchBox](Components-AlbumSearchBox)
- [AnchoredBubble](Components-AnchoredBubble)
- [AsciiTitle](Components-AsciiTitle)
- [BlockRange](Components-BlockRange)
- [Button](Components-Button)
- [Carousel](Components-Carousel)
- [ForumMessageBox](Components-ForumMessageBox)
- [Header](Components-Header)
- [HelpLink](Components-HelpLink)
- [MessageTextBox](Components-MessageTextBox)
- [PosterStats](Components-PosterStats)
- [Star](Components-Star)
- [Tips](Components-Tips)
- [UserMessages](Components-UserMessages)
- [UsernameLink](Components-UsernameLink)

### Feature Components

- [BeetsTerminal](Components-BeetsTerminal)
- [CoverArtTool](Components-CoverArtTool)
- [AsciiMan](Components-AsciiMan)
- [AvatarPreview](Components-AvatarPreview)
- [CarouselAlbums](Components-CarouselAlbums)
- [CarouselStickers](Components-CarouselStickers)
- [CreateList](Components-CreateList)
- [DesignTool](Components-DesignTool)
- [FilmClub](Components-FilmClub)
- [HomeIndex](Components-HomeIndex)
- [HomeTravel](Components-HomeTravel)
- [MessageBoard](Components-MessageBoard)
- [NowWatching](Components-NowWatching)
- [Oneko](Components-Oneko)
- [PlaceSticker](Components-PlaceSticker)
- [PlaceStickerCore](Components-PlaceStickerCore)
- [PlayerBar](Components-PlayerBar)
- [PrivateRoute](Components-PrivateRoute)
- [RecentLists](Components-RecentLists)
- [Stats](Components-Stats)
- [StickerGrid](Components-StickerGrid)
- [TravelRecommendationList](Components-TravelRecommendationList)
- [VisualiserDock](Components-VisualiserDock)
- [WeathrAnimation](Components-WeathrAnimation)

## Other

- [Media Management](Media-Management) (architecture overview)
- [Styling](Styling)
- [Firestore Structure](Firestore-Structure)
- [PWA](PWA)
- [Privacy Policy](Privacy-Policy)
